...
function finishConnection (socket, noteId, socketId) {
  ...
  if (menmenPerm.isEnabled() && socket.request.user && socket.request.user.logged_in) {
    const noteAlias = extractNoteIdFromSocket(socket)
    const username = menmenPerm.getUsernameFromUser(socket.request.user)
    if (noteAlias && username) {
      menmenPerm.warmCanEdit(noteAlias, username).then(function (canEdit) {
        socket.menmenCanEdit = canEdit
        if (!canEdit) {
          socket.emit('menmen-permission-denied', { canEdit: false })
        }
      }).catch(function () {
        socket.menmenCanEdit = false
        socket.emit('menmen-permission-denied', { canEdit: false })
      })
    }
  }
...
function ifMayEdit (socket, callback) {
  ...
  if (menmenPerm.isEnabled() && socket.request.user && socket.request.user.logged_in) {
    ...
    const username = menmenPerm.getUsernameFromUser(socket.request.user)
    if (noteAlias && username) {
      const cached = menmenPerm.getCachedCanEdit(noteAlias, username)
      if (cached === null) {
        // Cache miss: use last known socket context while revalidating in background
        if (typeof socket.menmenCanEdit === 'boolean') {
          mayEdit = mayEdit && socket.menmenCanEdit
        } else {
          mayEdit = false
        }
        menmenPerm.warmCanEdit(noteAlias, username).then(function (canEdit) {
        ...
        mayEdit = mayEdit && cached
        socket.menmenCanEdit = cached
      }
    } else {
      mayEdit = false
    }
  } else if (menmenPerm.isEnabled() && mayEdit) {
    mayEdit = false
  }
