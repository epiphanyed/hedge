...
const { buildMenmenI18n } = require('../../menmen-i18n')
const { noteNeedsMathJax } = require('../../menmen-mathjax')
...
exports.showNote = function (req, res, next) {
  noteUtil.findNote(req, res, function (note) {
    ...
    const menmenCustomUI = config.menmen && config.menmen.customUI
    return res.render('hedgedoc.ejs', {
      title,
      opengraph,
      cspNonce: res.locals.nonce,
      menmenNeedsMathJax: noteNeedsMathJax(extracted.markdown || body),
      menmenCustomUI,
      menmenI18n: menmenCustomUI ? buildMenmenI18n(res.__.bind(res)) : null,
      menmenAssetVersion: (config.menmen && config.menmen.assetVersion) || '2026091101'
    })
  }, null, true)
}
