...
function normalizeAvatarUrl (avatar) {
  ...
  if (!minio || !minio.endPoint) return avatar
  const protocol = minio.secure ? 'https' : 'http'
  const port = minio.port ? `:${minio.port}` : ''
  return `${protocol}://${minio.endPoint}${port}/${String(avatar).replace(/^\/+/, '')}`
}
...
function parseProfile (data) {
  ...
  let avatar
  if (config.oauth2.userProfileAvatarAttr) {
    try {
      avatar = extractProfileAttribute(data, config.oauth2.userProfileAvatarAttr)
