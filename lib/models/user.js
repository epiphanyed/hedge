...
function resolveMenmenAvatarUrl (url) {
  if (!url || /^https?:\/\//i.test(url)) return url
  const minio = config.minio
  if (!minio || !minio.endPoint) return url
  const protocol = minio.secure ? 'https' : 'http'
  const port = minio.port ? `:${minio.port}` : ''
  return `${protocol}://${minio.endPoint}${port}/${String(url).replace(/^\/+/, '')}`
}
    ...
    switch (profile.provider) {
      ...
      case 'oauth2':
        if (profile.photos && profile.photos.length > 0 && profile.photos[0].value) {
          photo = resolveMenmenAvatarUrl(profile.photos[0].value)
        } else if (profile.avatarUrl) {
          photo = resolveMenmenAvatarUrl(profile.avatarUrl)
        } else {
          photo = generateAvatarURL(profile.username)
        }
        break
