...
# 勿用 YARN_* 作 ARG/ENV 名：Yarn 会把其当作配置项（如 YARN_BIN → bin）
ARG HD_YARN_CJS=.yarn/releases/yarn-4.9.0.cjs
...
RUN printf '\nnpmRegistryServer: "%s"\nhttpTimeout: 600000\n' "${NPM_REGISTRY}" >> .yarnrc.yml \
    && node ${HD_YARN_CJS} install --immutable
COPY . .
RUN node ${HD_YARN_CJS} build
